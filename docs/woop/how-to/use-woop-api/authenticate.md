---
description: KalyChain authentication and authorization for JSON-RPC
tags:
  - private networks
---
...
methods. KalyChain verifies users using
...
Kaly creates JWT internally with
...
Using JSON-RPC authentication and authorization with [MetaMask](https://metamask.io/) is not supported.
    ...
    layer that provides SSL termination. Kaly does not provide a HTTPS connection natively.
...
!!! example "Password hash subcommand"
    ...
    === "Command"
        ...
        ```bash
        Kaly password hash --password=MyPassword
        ```
...
!!! important
    JWT public authentication disables the Kaly `/login` endpoint, meaning
